...
title_label = ttk.Label(root, text="YouTube mp3/mp4 dowlander")

insert_link_frame = ttk.LabelFrame(root, text="Insert YouTube link")
insert_link_entry = ttk.Entry(insert_link_frame, width=60)

file_format_frame = ttk.Frame(root)
is_mp3 = ttk.BooleanVar()
mp3_radio = ttk.Radiobutton(file_format_frame, text="mp3", variable=is_mp3, value=True)
mp4_radio = ttk.Radiobutton(file_format_frame, text="mp4", variable=is_mp3, value=False)

output_dir_frame = ttk.LabelFrame(root, text='Select path to save download files')
output_dir_entry = ttk.Entry(output_dir_frame, width=46)
